...
if($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if(empty($_GET)){
        try{
            $query = $readDB->prepare('select id, FirstName, Username, Email, UserActive, AccountType, LoginAttempts, Organization, Website, Tag1, Tag2, Tag3, Pexels from tblusers');
            $query->execute();
            ...
            while($row = $query->fetch(PDO::FETCH_ASSOC)) {
                $user = new User($row['id'], $row['FirstName'], $row['Username'], $row['Email'], $row['UserActive'], $row['AccountType'], $row['LoginAttempts'], $row['Organization'], $row['Website'], $row['Tag1'], $row['Tag2'], $row['Tag3'], $row['Pexels']);
        ...
        if (empty($userid) || $userid == $returned_userid) {
            try{
                $query = $readDB->prepare('select id, FirstName, Username, Email, UserActive, AccountType, LoginAttempts, Organization, Website, Tag1, Tag2, Tag3, Pexels from tblusers where id = :userid');
